...
class AdminDashboardView:
    ...
    def is_user_active_today(self, user):
        ...
        try:
            ...
            # Vérifier si l'utilisateur a une dernière activité
            ...
            logger.error(f"Erreur lors de la vérification de l'activité utilisateur: {e}")
            return False
    ...
    def _load_initial_data_async(self):
        ...
        try:
            ...
            # Charger les informations système en dernier
    ...
    def _get_recent_error_logs(self):
        """
        Récupère les journaux d'erreurs récents.
        
        Returns:
            list: Liste des journaux d'erreurs récents
        """
        try:
            # Essayer de récupérer les journaux d'erreurs depuis le modèle
            if hasattr(self.model, 'get_error_logs'):
                return self.model.get_error_logs()
            
            # Si le modèle a un attribut pour les journaux d'erreurs
            if hasattr(self.model, 'error_logs'):
                return self.model.error_logs
            
            # Si aucune des méthodes ci-dessus n'est disponible, générer des données de démo
            # Les journaux de démo sont générés pour les dernières 48 heures
            now = datetime.now()
            demo_logs = []
            
            # Générer quelques entrées de journal de démo
            for i in range(10):
                hours_ago = (i * 5) % 48  # Répartir sur 48 heures
                log_time = now - timedelta(hours=hours_ago)
                
                demo_logs.append({
                    "timestamp": log_time,
                    "level": "ERROR" if i % 3 == 0 else "WARNING",
                    "message": f"Erreur de démonstration #{i+1}",
                    "source": "système" if i % 2 == 0 else "application"
                })
            
            return demo_logs
            
        except Exception as e:
            logger.error(f"Erreur lors de la récupération des journaux d'erreurs: {e}")
            return []
    
    def _is_within_last_24h(self, log):
        """
        Vérifie si un journal d'erreur est dans les dernières 24 heures
        
        Args:
            log: Journal d'erreur à vérifier
            
        Returns:
            bool: True si le journal est dans les dernières 24 heures, False sinon
        """
        try:
            now = datetime.now()
            log_time = log.get("timestamp")
            
            # Si log_time est une chaîne, la convertir en datetime
            ...
            # Vérifier si le journal est dans les dernières 24 heures
            time_diff = now - log_time
            return time_diff.total_seconds() < 86400  # 24 heures en secondes
            
        except Exception as e:
            logger.warning(f"Erreur lors de la vérification de la date du journal: {e}")
            return False
    ...
    def _update_cache_timestamp(self, key: str):
        """
        Met à jour le timestamp du cache pour une clé donnée
        
        Args:
            key: Clé du cache à mettre à jour
        """
        try:
            if key in self._cache:
            ...
            logger.error(f"Erreur lors de la mise à jour du timestamp du cache: {e}")